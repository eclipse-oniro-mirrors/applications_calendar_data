...
CJNativeCalendar::CJNativeCalendar(int id)
    : m_id(id)
{
...
void CJNativeCalendar::InsertReminders(int eventId, vector<int> reminders)
{
    ...
    for (const auto &reminder : reminders) {
            ...
            auto index = CJDataShareHelperManager::GetInstance().Insert(*(m_reminderUrl.get()), valuesBucket);
...
int CJNativeCalendar::AddEventInfo(const Event& event, int channelId)
{
    ...
    if (valueAttendees.size() > 0) {
        auto count = CJDataShareHelperManager::GetInstance().BatchInsert(*(m_attendeeUri.get()), valueAttendees);
...
int CJNativeCalendar::AddEvents(const std::vector<Event>& events)
{
    std::vector<DataShare::DataShareValuesBucket> valueEvents;
    for (const auto &event : events) {
        valueEvents.emplace_back(BuildValueEvent(event));
    }
    auto count = CJDataShareHelperManager::GetInstance().BatchInsert(*(m_eventUri.get()), valueEvents);
...
bool CJNativeCalendar::DeleteEvent(int id)
{
    DataShare::DataSharePredicates predicates;
    predicates.EqualTo("_id", id);
    predicates.EqualTo("calendar_id", GetId());
    auto ret = CJDataShareHelperManager::GetInstance().Delete(*(m_eventUri.get()), predicates);
    return ret == 1;
}

void CJNativeCalendar::DeleteAllEvents()
{
    DataShare::DataSharePredicates predicates;
...
int CJNativeCalendar::DeleteEvents(const std::vector<int>& ids)
{
    int count = 0;
...
bool CJNativeCalendar::UpdateEvent(const Event& event)
{
    if (!event.id) {
    ...
    auto ret = CJDataShareHelperManager::GetInstance().Update(*(m_eventUri.get()), m_predicates, valueEvent);
    LOG_INFO(" Update code %{public}d", ret);
    {
        // delete attendee
        DataShare::DataSharePredicates predicates;
        predicates.EqualTo("event_id", eventId);
        auto ret = CJDataShareHelperManager::GetInstance().Delete(*(m_attendeeUri.get()), predicates);
    ...
    
    {
        ...
        DataShare::DataSharePredicates predicates;
        predicates.EqualTo("event_id", eventId);
        auto ret = CJDataShareHelperManager::GetInstance().Delete(*(m_reminderUrl.get()), predicates);
...
int CJNativeCalendar::UpdateEvents(const std::vector<Event>& events)
{
    int count = 0;
    for (const auto &event : events) {
...
std::vector<Attendee> CJNativeCalendar::GetAttendeesByEventId(int id)
{
    DataShare::DataSharePredicates predicates;
    ...
    auto result = CJDataShareHelperManager::GetInstance().Query(*(m_attendeeUri.get()), predicates, columns, &error);
...
std::optional<std::vector<int>> CJNativeCalendar::GetRemindersByEventId(int id)
{
    DataShare::DataSharePredicates predicates;
    predicates.EqualTo("event_id", id);
    std::vector<std::string> columns = {"event_id", "minutes"};
    DataShare::DatashareBusinessError error;
    auto result = CJDataShareHelperManager::GetInstance().Query(*(m_reminderUrl.get()), predicates, columns, &error);
    if (result == nullptr) {
        return std::nullopt;
    }
...
std::vector<Event> CJNativeCalendar::GetEvents(std::shared_ptr<EventFilter> filter, const std::vector<string>& eventKey)
{
    ...
    DataShare::DatashareBusinessError error;
    auto result = CJDataShareHelperManager::GetInstance().Query(*(m_eventUri.get()),
...
void CJNativeCalendar::FillEventsInfo(std::vector<Event> &events, const std::set<std::string>& resultSetField)
{
    std::vector<Attendee> attendees;
...
std::vector<Event> CJNativeCalendar::QueryEventInstances(int64_t start, int64_t end, const std::vector<int> &ids,
    const std::vector<string>& eventKey)
{
    ...
    DataShare::DatashareBusinessError error;
    auto result = CJDataShareHelperManager::GetInstance().Query(*(m_instanceUrl.get()),
        *(predicates.get()), queryField, &error);
    if (!result) {
        LOG_ERROR("query failed");
        return events;
...
CalendarConfig CJNativeCalendar::GetConfig()
{
    DataShare::DataSharePredicates predicates;
    predicates.EqualTo("_id", m_id);
    std::vector<std::string> columns = {"calendar_color", "canReminder"};
    DataShare::DatashareBusinessError error;
    auto result = CJDataShareHelperManager::GetInstance().Query(*(m_calendarUri.get()), predicates, columns, &error);
    if (result != nullptr) {
...
bool CJNativeCalendar::SetConfig(const CalendarConfig& config)
{
    DataShare::DataSharePredicates m_predicates;
    ...
    auto ret = CJDataShareHelperManager::GetInstance().Update(*(m_calendarUri.get()), m_predicates, valuesBucket) == 1;
